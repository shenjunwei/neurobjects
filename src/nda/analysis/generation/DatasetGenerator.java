    ...
    throws GenerationException {
        ...
        if (DatasetTransformer.needsSpikeTrainTransform(dataset)) {
            ...
            for (int round = 1; round <= dataset.getNumberRounds(); ++round) {
                ...
                BehaviorHandlerI roundHandler = globalBehaviorHandler;

                if (DatasetTransformer.needsBehaviorHandlerTransform(dataset))
                    roundHandler = DatasetTransformer.applyBehaviorHandlerTransform(
                            randomData, dataset, roundHandler);

                if (DatasetTransformer.needsVarianceContactsTransform(dataset))
                    roundHandler = DatasetTransformer.applyVarianceContactsTransform(
                            randomData, dataset, roundHandler);
